...
merchant_name = csv_reader['Merchant_Name'].tolist()
...
for i in range(total_row):
    ...
    time.sleep(3)
    driver.find_element_by_xpath("//input[@placeholder='Search merchant....']").send_keys(merchant_name[i], Keys.ENTER)
    time.sleep(7)
    driver.find_element_by_xpath("//tbody/tr/td[@class='p-2']/a").click()
    time.sleep(2)


    CreatePrimaryShop = driver.find_element_by_xpath("//button/span[contains(text(),'Create Primary Shop')]")
